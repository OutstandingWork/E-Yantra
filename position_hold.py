...
class Edrone():
	...
	def __init__(self):
		...
		self.throttle_error=0  #self.error_values=[0,0,0] 
		self.throttle_prev_error=0 #self.prev_values=[0,0,0]
		self.throttle_sum_error=0 #self.throttle_values=[0,0,0]
		self.min_throttle=1000 #self.min_values=[1000,1000,1000]
		self.max_throttle=2000 #self.max_values[2000,2000,2000]
 ...
 #arm and disarm are basically switches which don't listen to other codeparts apart from AUX values written above
	...
	#	8. Add error_sum
		...
		self.cmd.rcThrottle=1500+int(self.throttle_error*self.Kp[2]+(self.throttle_error-self.throttle_prev_error)*self.Kd[2]+self.throttle_sum_error*self.Ki[2])
